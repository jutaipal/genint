...
/*                                                                                                                                    */
/*  NEW OPTIONS (defaults unchanged: generate mode, random threshold):                                                                 */
/*    -keepfeatures : with -file, keep all matched windows (all PWMs, both strands), re-sample every other base from the background   */
/*                    PWM, and rescan/re-sample until no new match is found. Reads must have length = background PWM width.           */
/*    -threshold    : fixed match threshold scale/(1+scale) instead of the random Scaled_probability() threshold (both modes).       */
/*                    Note: a PWM whose maximum score is <= threshold can never match (generate mode would then not terminate).       */
/*                                                                                                                                    */
/*  TESTS PERFORMED (Sep 2026, gcc, AddressSanitizer + UndefinedBehaviorSanitizer, valgrind memcheck):                                 */
/*    - Unit tests: Load_pwm, Normalize_pwm, Scaled_probability CDF, Match_threshold (value and RNG consumption), Window_score         */
/*      (exact vs original arithmetic, all positions incl. edges, reverse == forward of revcomp), find_and_improve_best_match.        */
/*    - Default output byte-identical to previous version with fixed seed (generate, -nomutate, -file).                                */
/*    - -keepfeatures: features match an independent Python scan; protected bases unchanged; no new matches in output; recycling      */
/*      output gives identical features; edge, overlapping and reverse-strand features; length and base==4 guards.                     */
/*    - Mutation testing: 10 injected off-by-one / logic bugs, all detected.                                                           */
/*    - valgrind: no memory errors; leaks identical to previous version (pre-existing allocations only).                               */
...
/* NEW (-threshold): if set, every match test uses a fixed threshold instead of a random one. */
static int use_fixed_threshold = 0;

/* NEW: threshold used by every match test.
   Default: random threshold, identical to calling Scaled_probability() directly.
   -threshold: Scaled_probability() evaluated at p = 0.5, i.e. odds = scale, threshold = scale/(1+scale). */
static inline long double Match_threshold(long double scale) {
    if (use_fixed_threshold) return scale / (1.0L + scale);
    return Scaled_probability(scale);
}

/* NEW (-keepfeatures): score of the window covering sequence[pos] .. sequence[pos + width - 1].
   Same arithmetic as the existing file-mode loop:
   forward: product of fraction[nuc][i];  reverse: product of fraction[3 - nuc][width - 1 - i]. */
static long double Window_score(struct normalized_pwm *p, short int *sequence, int pos, int is_reverse) {
    long double score = 1.0;
    for (int i = 0; i < (*p).width; i++) {
        int nuc = sequence[pos + i];
        if (is_reverse) score *= (*p).fraction[3 - nuc][(*p).width - 1 - i];
        else            score *= (*p).fraction[nuc][i];
    }
    return score;
}
...
int find_and_improve_best_match(short int* sequence_value, struct normalized_pwm* qs, int num_signal_pwms, int Nlength, int* match_position, long double* final_random_number, long double* final_score, int* matching_pwm, int improve_by_mutation, long int sequences_to_print) {
   ...
   if (!improve_by_mutation) {
       long double current_random = Match_threshold(qs[best_pwm].scale_factor);
...

    {
        ...
        /* ---- 2.  acceptance test ------------------------------------- */
        long double thr = Match_threshold(qs[best_pwm].scale_factor);
...
int main (int argc, char *argv[]) {
    ...
    // NEW: Check for -keepfeatures flag (same stripping as -nomutate)
    int keep_features = 0;
    for(int i = 1; i < argc; i++) {
        if(strcmp(argv[i], "-keepfeatures") == 0) {
            keep_features = 1;
            for(int j = i; j < argc-1; j++) {
                argv[j] = argv[j+1];
            }
    ...
    // NEW: Check for -threshold flag (same stripping as -nomutate)
    for(int i = 1; i < argc; i++) {
    ...
    if(use_input_file) {
        ...
        // NEW (-keepfeatures): buffers, allocated once.
        // keep_mask[i] = 1 means base i lies inside at least one matched window and must not be changed.
        // A read has at most (num_signal_pwms * 2 strands * read length) matched windows.
        #define KEEPFEATURES_MAX_ATTEMPTS 10000
        unsigned char *keep_mask = NULL;
        int *feature_pwm = NULL, *feature_pos = NULL, *feature_strand = NULL;
        int max_features = num_signal_pwms * 2 * Nlength;
        long reads_skipped_length = 0;
        long reads_hitting_attempt_limit = 0;
        if(keep_features) {
            keep_mask      = malloc(Nlength * sizeof(unsigned char));
            feature_pwm    = malloc(max_features * sizeof(int));
            feature_pos    = malloc(max_features * sizeof(int));
            feature_strand = malloc(max_features * sizeof(int));
            if(!keep_mask || !feature_pwm || !feature_pos || !feature_strand) { perror("malloc"); exit(3); }
        }
        ...
        while(fgets(line, sizeof(line), seq_file) && number_of_generated_sequences > 0) {
            ...
            // NEW (-keepfeatures): every base needs its own background PWM column, and
            // sequence_value holds Nlength-1 bases, so read length must equal background PWM width.
            if(keep_features && (len != Nlength - 1 || len >= max_width_of_pwm)) {
                fprintf(stderr, "Warning: skipping read of length %d (-keepfeatures requires length %d)\n", len, Nlength - 1);
                reads_skipped_length++;
                continue;
            }
            ...
            if(keep_features) {
                // ===================== NEW: -keepfeatures =====================
                int read_length = actual_len;   // == Nlength - 1, enforced above
                int num_features = 0;
                
                // ---- Step 1: find ALL matched windows (all PWMs, both strands, every position, no early break)
                for(int i = 0; i < read_length; i++) keep_mask[i] = 0;
                
                for(int p = 0; p < num_signal_pwms; p++) {
                    int width = qs[p].width;
                    if(read_length < width) continue;
                    int pwm_has_feature = 0;
                    // window starting at 'start' covers bases start .. start + width - 1 (inclusive)
                    // last valid start = read_length - width, so the last window ends at base read_length - 1
                    for(int start = 0; start <= read_length - width; start++) {
                        for(int strand = 0; strand < 2; strand++) {   // 0 = forward, 1 = reverse
                            long double window_score = Window_score(&qs[p], sequence_value, start, strand);
                            long double threshold = Match_threshold(qs[p].scale_factor);
                            if(threshold < window_score) {
                                feature_pwm[num_features]    = p;
                    ...
                    if(pwm_has_feature) match_counts[p]++;
                }
                
                // ---- Step 2: re-sample unprotected bases, rescan, repeat until no new match
                int attempts = 0;
                int new_match_found;
                do {
                    attempts++;
                    
                    // Re-sample every unprotected base from the background PWM (same loop as generate mode)
                    for(int i = 0; i < read_length; i++) {
                        if(keep_mask[i]) continue;
                        current_random_number = xoshiro_to_long_double();
                        for(cutoff = 0, base = 0; base < 4; base++) {
                            cutoff += qb.fraction[base][i];
                            if (current_random_number < cutoff) break;
                        }
                        if (base == 4) base = 3;   // numeric guard (as in mutation block): avoids fraction[3-4] in reverse scoring
                        sequence_value[i] = base;
                    }
                    ...
                    for(int p = 0; p < num_signal_pwms && !new_match_found; p++) {
                        int width = qs[p].width;
                        if(read_length < width) continue;
                        for(int start = 0; start <= read_length - width && !new_match_found; start++) {
                            int contains_resampled_base = 0;
                            for(int i = start; i < start + width; i++) {
                                if(!keep_mask[i]) { contains_resampled_base = 1; break; }
                            }
                            if(!contains_resampled_base) continue;
                            for(int strand = 0; strand < 2 && !new_match_found; strand++) {
                                long double window_score = Window_score(&qs[p], sequence_value, start, strand);
                                long double threshold = Match_threshold(qs[p].scale_factor);
                                if(threshold < window_score) new_match_found = 1;
                            }
                        }
                    }
                } while(new_match_found && attempts < KEEPFEATURES_MAX_ATTEMPTS);
                
                if(new_match_found) reads_hitting_attempt_limit++;
                
                // ---- Step 3: print sequence, attempts, and kept features (PWM, start position, F/R)
                printf("\n");
                for(int i = 0; i < read_length; i++) printf("%c", DNA[sequence_value[i]]);
                printf("\t%d", attempts);
                if(num_features == 0) printf("\tNo features");
                for(int f = 0; f < num_features; f++) {
                    printf("\t%s\t%i%c", qs[feature_pwm[f]].name, feature_pos[f], "FR"[feature_strand[f]]);
                }
                if(new_match_found) printf("\tATTEMPT_LIMIT_REACHED");
                // =================== end NEW: -keepfeatures ===================
            } else
            // Original counting loop (unchanged)
            ...
            for(int pwm_idx = 0; pwm_idx < num_signal_pwms; pwm_idx++) {
                ...
                for(int pos = 0; pos <= actual_len - qs[pwm_idx].width && !found_match; pos++) {
                    ...
                    long double threshold = Match_threshold(qs[pwm_idx].scale_factor);
                    ...
                    threshold = Match_threshold(qs[pwm_idx].scale_factor);
        ...
        // NEW (-keepfeatures): summary and cleanup
        if(keep_features) {
            printf("Reads skipped (wrong length): %ld\n", reads_skipped_length);
            printf("Reads that reached %d attempts with a new match still present: %ld\n",
                   KEEPFEATURES_MAX_ATTEMPTS, reads_hitting_attempt_limit);
            free(keep_mask);
            free(feature_pwm);
            free(feature_pos);
            free(feature_strand);
